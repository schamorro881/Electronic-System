...
@Component({
  ...
  template: `
    <div class="sidebar-backdrop" *ngIf="layoutService.isMobileMenuOpen()" (click)="layoutService.closeMobileMenu()"></div>
    <nav class="sidebar glass" [class.collapsed]="layoutService.isCollapsed()" [class.mobile-open]="layoutService.isMobileMenuOpen()">
      ...
      <div class="footer-auth tui-space_top-6">
        <a tuiLink routerLink="/auth/login" class="login-link glass-light" [title]="layoutService.isCollapsed() ? 'Iniciar Sesión' : ''" (click)="layoutService.closeMobileMenu()">
  ...
  styles: [`
    ...
    @media (max-width: 768px) {
      ...
      .sidebar.mobile-open {
        ...
        box-shadow: 10px 0 40px rgba(0, 0, 0, 0.8), 1px 0 0 rgba(99, 102, 241, 0.3);
      }
      .sidebar-backdrop {
        position: fixed;
